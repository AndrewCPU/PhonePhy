...
//
    ...
    public void addPhoneNumber(int a, int b,  int c) throws Exception
    {
        ...
        Statement st = con.createStatement();
        String built = "(" + a + ") " + b + "-" + c;
        String values = " VALUES ( '" + built + "', '" + a + "', '" + b + "', '" + c + "');";
        st.executeUpdate("INSERT INTO `phone_numbers`.`numbers` (`built`, `area`, `first`, `second`)" + values);
        System.out.print(built + "\n");

    }
    public void claimAreaCode(int area) throws Exception
    {
        if(con==null) {
            con = DriverManager.getConnection(url, user, password);
        }
        update();
        Statement st = con.createStatement();
        String name = java.net.InetAddress.getLocalHost().getHostName() + "<::>" + System.getProperty("user.name");
        String values = " VALUES ( '" + area + "', '" + name + "');";
        st.executeUpdate("INSERT INTO `phone_numbers`.`claimed_area_codes` (`area_code`, `computer_name`)" + values);
